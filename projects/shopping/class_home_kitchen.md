# Class Authority: Home & Kitchen

## Purpose
Own behavior shared broadly by Home & Kitchen products when the `Home & Kitchen` Class is active.

Use the Home & Kitchen sections of `criteria.md`, `source_playbooks.md`, and `pricing.md` alongside this file.

This Class mainly contributes additive household-use behavior. It should not invent workflow overrides unless a generic workflow rule is genuinely wrong across multiple Home & Kitchen categories.
...
## Boundary
This file contains only behavior genuinely reusable across multiple Home & Kitchen categories. Kitchen-knife behavior belongs to `category_kitchen_knives.md`; universal Shopping behavior belongs to `base.md`; workflow Merge/Override semantics belong to `product_hierarchy.md`.
