...
Do not switch product forms casually. Resolve only to a named Type that actually exists in `product_hierarchy.md`. If the better adjacent format has no live Type authority yet, fall back to the `Kitchen Knives` Category context and evaluate the product form there; do not invent a Type, specialized file, or inherited rules that do not exist.
...
Treat that point as the Chef's-Knife functional sweet spot while preserving higher craft/refinement value when relevant under `pricing.md` and the inherited Kitchen Knife guidance.
...
## Boundary
This file owns Chef's-Knife-specific shared design-family context and workflow deltas only. Criterion meaning belongs to `criteria.md`; reusable value interpretation to `pricing.md`; source discovery to `source_playbooks.md`; shared metallurgy/sharpening/construction/provenance rules to `category_kitchen_knives.md`; Merge/Override semantics and child-delta rules to `product_hierarchy.md`.
